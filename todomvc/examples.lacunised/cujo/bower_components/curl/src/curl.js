...
//"use strict"; don't restore this until the config routine is refactored
	...
	function Promise () {
		...
		complete = function promiseComplete (success, arg) {
			...
			then = success ?
				...
				function (resolved, rejected) {lacuna_lazy_load("bower_components/curl/src/curl.js[5219:5249]", functionData => eval(functionData))};
		...
		this.reject = function (ex) {lacuna_lazy_load("bower_components/curl/src/curl.js[5793:5845]", functionData => eval(functionData))};
	...
	core = {
		...
		getCjsModule: function (def) {
			...
			if (!module) {
				module = def.module = {
					...
					'config': function () {lacuna_lazy_load("bower_components/curl/src/curl.js[11948:11970]", functionData => eval(functionData))}
				};
		...
		loadScript: function (def, success, failure) {
			...
			function fail (e) {lacuna_lazy_load("bower_components/curl/src/curl.js[20878:21071]", functionData => eval(functionData))}
		...
		getDeps: function (parentDef) {
			...
			function getDep (name, index) {
				...
				when(childDef,
					resolveOnce,
					parentDef.reject,
					parentDef.exports && function (msg) {lacuna_lazy_load("bower_components/curl/src/curl.js[27057:27484]", functionData => eval(functionData))}
				);
			}
	...
	function _config (cfg, callback, errback) {
		...
		if (cfg) {
			...
			if (main) {
				...
				fallback = main[1]
					? function () {lacuna_lazy_load("bower_components/curl/src/curl.js[35326:35386]", functionData => eval(functionData))}
					: mPromise.reject;
	...
	function CurlApi (ids, callback, errback, waitFor, isPreload) {
		...
		this['then'] = this.then = then = function (resolved, rejected) {
			when(ctx,
				...
				function (ex) {lacuna_lazy_load("bower_components/curl/src/curl.js[36012:36067]", functionData => eval(functionData))}
			);
			return this;
		};

		this['next'] = function (ids, cb, eb) {lacuna_lazy_load("bower_components/curl/src/curl.js[36136:36198]", functionData => eval(functionData))};
	...
	cache['curl/_privileged'] = {
		'core': core,
		'cache': cache,
		'config': function () {lacuna_lazy_load("bower_components/curl/src/curl.js[38966:38985]", functionData => eval(functionData))},
		'_define': _define,
		'_curl': _curl,
		'Promise': Promise
	};
